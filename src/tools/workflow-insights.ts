/**
 * Workflow-insights tools — curated, read-only, composed analytics over the
 * public GHL v2 API (ported from Go-High-Level-MCP-2026).
 *
 * Each tool orchestrates several public endpoints through the per-sub-account
 * CRMClient and returns a summarized view. The heavy lifting lives in the
 * WorkflowInsights helper; the ToolDefs at the bottom instantiate it per call.
 */
...
class WorkflowInsights {
  constructor(private client: CRMClient) {}

  /** Adapt CRMClient (throws on error, returns raw data) to the {success,data} shape. */
  private async makeRequest(method: string, endpoint: string, body?: unknown): Promise<GHLResult> {
    const data = await this.client.request(method as any, endpoint, body !== undefined ? { data: body } : {});
  ...
  private async summarizeEmailCampaignPerformance(args: Record<string, unknown>): Promise<unknown> {
    ...
    const limit = this.limit(args.limit, 25, 100);
    const includeRaw = args.includeRaw === true;
    const params = new URLSearchParams({ locationId, campaignsOnly: 'true', showStats: 'true', limit: String(limit) });
    ...
    const stats = await Promise.all(
      campaigns.map(async campaign => {
        ...
        const recipientOutcome =
          args.includeRecipients === true && id
  ...
  private async findUncontactedFormLeads(args: Record<string, unknown>): Promise<unknown> {
    ...
    for (const formId of formIds) {
      ...
      for (const submission of this.firstArray(submissions.response?.data, ['submissions', 'data', 'items']).slice(0, limit)) {
        ...
        const contactSearch = query ? await this.safePost('/contacts/search', { locationId, pageLimit: 1, query }) : undefined;
    ...
    return this.withRaw({ success: true, locationId, formsChecked: formIds.length, uncontactedCount: leads.length, leads }, raw, includeRaw);
  }
  ...
  private locationId(args: Record<string, unknown>): string {
    const locationId = (args.locationId as string | undefined) || this.client.locationId;
...
export const workflowInsightsTools: ToolDef[] = [
  defineTool({
    name: 'audit_location_ads_setup',
    description:
      'Read-only audit of location setup relevant to paid-ad lead routing, forms, workflows, calendars, tags, and email follow-up.',
    properties: {
      includeRaw: { type: 'boolean', description: 'Include raw endpoint outcomes for deeper inspection.' },
      limit: { type: 'number', description: 'Maximum records to sample from each collection (default: 25, max: 100).' },
    },
    handler: (client, args) => new WorkflowInsights(client).handleToolCall('audit_location_ads_setup', args),
  }),
  defineTool({
    name: 'summarize_email_campaign_performance',
    description:
      'Summarize scheduled email campaign performance for a location, optionally sampling per-campaign stats endpoints.',
    properties: {
      ...
      includeRecipients: { type: 'boolean', description: 'Also sample recipient counts by status for each campaign.' },
      includeRaw: { type: 'boolean', description: 'Include raw endpoint outcomes.' },
    },
    handler: (client, args) => new WorkflowInsights(client).handleToolCall('summarize_email_campaign_performance', args),
  }),
  defineTool({
    name: 'find_uncontacted_form_leads',
    description:
      'Find recent form submissions that do not appear to have conversation activity, using form submissions, contact search, and conversation search.',
    properties: {
    ...
    handler: (client, args) => new WorkflowInsights(client).handleToolCall('find_uncontacted_form_leads', args),
  }),
  defineTool({
    ...
    handler: (client, args) => new WorkflowInsights(client).handleToolCall('summarize_calendar_availability', args),
  }),
];
